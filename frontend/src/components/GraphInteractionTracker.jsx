...
const GraphInteractionTracker = ({
  ...
  const lastActiveTimeRef = useRef(null); // Sync external counts when they change
  ...
  }, [externalDislikeCount]); // Track mouse enter/leave to measure active time
  ...
  }, [containerRef]); // Update time spent every second while active
  ...
  }, [containerRef]); // Track clicks on Plotly's default export button (optional)
  ...
  }, [containerRef]); // Send interaction data to backend on unload or visibility change
  ...
  }, [isActive, exportCount, likeCount, dislikeCount, panCount, graphName]); // Display interaction stats below graph
  ...
  return (
    ...
    //        {" "}
    //   <div>
    //     <strong>{graphName}</strong> Interaction Summary
    //   </div>
    //        {" "}
    //   <div>
    //     Time spent interacting: {timeSpentSec} second
    //     {timeSpentSec !== 1 ? "s" : ""}
    //   </div>
    //         <div>Exported times: {exportCount}</div>     {" "}
    //   <div>Likes: {likeCount}</div>      <div>Dislikes: {dislikeCount}</div>
